...
if($arr['action'] === 'ADD') {
    ...
    //if($result->num_rows === 0) {
        $sql = $connection->prepare("INSERT INTO `pancakes` (`name`, `message`) VALUES (?,?);");
        $requestsql = $sql->bind_param('ss', $arr['name'], $arr['message']);
...
if($arr['action'] === 'GET') {
    $sqla = $connection->prepare("SELECT * FROM `pancakes` LIMIT 50");
    //$requestsqla = $sqla->bind_param('i', $arr['xid']);
    $sqla->execute();
    $result = $sqla->get_result();
    $data = [];
